...
public class DiagramHistory {
	...
	private LinkedList<DiagramHistoryElement> _undoStack = new LinkedList<DiagramHistoryElement>();
	
	/** The future diagrams after the current */
	private LinkedList<DiagramHistoryElement> _redoStack = new LinkedList<DiagramHistoryElement>();
	 ...
	 */
	public DiagramHistory(int capacity) {
		_capacity = capacity;
	 ...
	 * @param capacity The maximum number of history allowed.
	 */
	public DiagramHistory(int capacity, Diagram initialDiagram) {
		_capacity = capacity;
	...
	public void add(Diagram newDiagram, String message) {
		...
		DiagramHistoryElement newElement = new DiagramHistoryElement(_current, message);
	...
	public Diagram nextRedo() {
		if(hasNextRedo()) {
			// Obtain the previous diagram and the log message.
			DiagramHistoryElement element = _redoStack.removeLast();
			Diagram newDiagram = element.getDiagram();
			String message = element.getMessage();
			
			// Put the current diagram into the redo stack.
			_undoStack.addLast(new DiagramHistoryElement(_current, message));
